...
import {
  createPaymentIntegrationSchema,
  getPaymentIntegrationDefaultValues,
  type PaymentIntegrationFormValues,
} from "../schemas/payment-integration.schema"
...
type PaymentIntegrationFormProps = {
  provider: ApiIntegrationProviderConfig
}

function PaymentLogoField({
  file,
  existingFileName,
  onChange,
  disabled,
  ...
  return (
    <Field>
      <FieldLabel htmlFor="payment-integration-logo">Choose logo</FieldLabel>
      <div
        className={cn(
...
export function PaymentIntegrationForm({ provider }: PaymentIntegrationFormProps) {
  const { updateIntegrationProvider, isPending } = useUpdateIntegrationProvider()

  const form = useForm<PaymentIntegrationFormValues>({
    resolver: zodResolver(createPaymentIntegrationSchema(provider.has_secret_key)),
    defaultValues: getPaymentIntegrationDefaultValues(provider),
  })
  ...
  async function onSubmit(values: PaymentIntegrationFormValues) {
    ...
    >
      ...
      <FieldGroup className="gap-5">
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="payment-secret-key">
                Secret key{" "}
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="payment-public-key">
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="payment-hash">
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="payment-gateway-title">
