...
function ComboboxClear({ className, ...props }: ComboboxPrimitive.Clear.Props) {
  return (
    <ComboboxPrimitive.Clear
      ...
      {...props}
    >
      <Icons.close size={16} className="pointer-events-none" />
    </ComboboxPrimitive.Clear>
  )
}
...
function ComboboxContent({
  ...
  return (
    <ComboboxPrimitive.Portal container={overlayContainer ?? undefined}>
...
function ComboboxItem({
  ...
  return (
    ...
    >
      ...
      >
        <Icons.check size={16} className="pointer-events-none" />
      </ComboboxPrimitive.ItemIndicator>
    </ComboboxPrimitive.Item>
  )
}
...
function ComboboxChips({
  ...
  return (
    <ComboboxPrimitive.Chips
      ...
      className={cn(
        "flex min-h-8 flex-wrap items-center gap-1 rounded-2xl border border-transparent bg-input/50 bg-clip-padding px-2.5 py-1 text-sm transition-[color,box-shadow] duration-200 focus-within:border-ring has-aria-invalid:border-destructive has-data-[slot=combobox-chip]:px-1 dark:has-aria-invalid:border-destructive/50",
        className
      )}
      {...props}
    />
