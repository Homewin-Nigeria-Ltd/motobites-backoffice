...
import { useCreateInventoryItem } from "@/features/inventory/hooks/use-inventory-mutations"
import type { InventoryStockLevel } from "@/features/inventory/types"
import {
  inventoryItemFormDefaults,
...
type AddInventoryItemSheetProps = {
  open: boolean
...
function AddInventoryItemForm({
  ...
  const { createItem, isPending } = useCreateInventoryItem()
  const [image, setImage] = useState<File | null>(null)

  const form = useForm<InventoryItemFormValues>({
    resolver: zodResolver(inventoryItemFormSchema),
    defaultValues: inventoryItemFormDefaults,
  })
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    createItem(payload).then((result) => {
      ...
      form.reset(inventoryItemFormDefaults)
      setImage(null)
      onOpenChange(false)
    })
  })
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <Field>
        ...
        <ImageUpload
          ...
          existingImageAlt={itemName || "Inventory item image"}
          onChange={setImage}
          disabled={isPending}
        />
      </Field>
...
export function AddInventoryItemSheet({
  ...
  return (
    <SlideInModal
      ...
      closeLabel="Close add item form"
      bodyClassName="space-y-5"
    >
      {open ? (
        <AddInventoryItemForm key="add-inventory-item" onOpenChange={onOpenChange} />
      ) : null}
    </SlideInModal>
  )
}
